...
def main():
    ...
    try:
        ...
        lv95_lhn02_to_ecef = Transformer.from_crs("EPSG:2056+5728", "EPSG:4978", always_xy=True)
    except ImportError:
    ...
    # Use a default fallback for global anchor Z if we don't have exactly one.
    _, _, anchor_alt = lv95_lhn02_to_wgs84_3d.transform(global_cx, global_cy, 300.0)
    ...
    for g in grids:
        ...
        SELECT 
            citydb_objectid,
            ST_AsGeoJSON(ST_Force3D((ST_Dump(citydb_geometry)).geom)) as geom
        ...
        tile_alt_ln02 = min(all_alts) if all_alts else 300.0
        
        # Now recalculate ECEF parameters using the correct terrain altitude anchor for this specific tile!
        _, _, tile_alt = lv95_lhn02_to_wgs84_3d.transform(g["cx"], g["cy"], tile_alt_ln02)
                ...
                for poly in polygons:
                    for ring in poly:
                        ...
                        for p in pts:
                            ...
                            alt_ln02 = p[2] if len(p) > 2 else tile_alt_ln02
                            
                            # Standardize direct ECEF
                            vx, vy, vz = lv95_lhn02_to_ecef.transform(easting, northing, alt_ln02)
    ...
    # Build root transform: We define the global anchor simply using standard ENU to ECEF 
    # without manually applying Rx(-90) here. Cesium natively handles Y-up glTF to Z-up ECEF conversion!
    enu_to_ecef = [

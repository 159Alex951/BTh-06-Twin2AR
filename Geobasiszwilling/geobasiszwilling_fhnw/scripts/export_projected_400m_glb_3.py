...
def main():
    ...
    try:
        ...
        # LV95 (easting/northing) + LHN02 orthometric height -> WGS84 3D (lon/lat/ellipsoidal height)
        # This correctly applies the Swiss geoid model (~+47-50m offset in Basel area)
        lv95_lhn02_to_wgs84_3d = Transformer.from_crs("EPSG:2056+5728", "EPSG:4979", always_xy=True)
    except ImportError:
    ...
    # Query a representative LHN02 terrain height near the global anchor centre
    anchor_z_query = f"""
    SELECT AVG(z_base) FROM citydb.projected_buildings_2d
    WHERE ST_DWithin(geom, ST_SetSRID(ST_Point({global_cx}, {global_cy}), 2056), 1000)
    """
    anchor_z_res = run_query(anchor_z_query, args)
    anchor_z_lhn02 = float(anchor_z_res.strip()) if anchor_z_res and anchor_z_res.strip() else 300.0
    # Convert LHN02 orthometric height to WGS84 ellipsoidal height
    _, _, anchor_alt = lv95_lhn02_to_wgs84_3d.transform(global_cx, global_cy, anchor_z_lhn02)
    print(f"Anchor altitude: LHN02={anchor_z_lhn02:.2f}m -> ellipsoidal={anchor_alt:.2f}m")
    ...
    for g in grids:
        ...
        tile_alt_lhn02 = min_alt if min_alt != float('inf') else 300.0
        # Convert LHN02 orthometric height to WGS84 ellipsoidal height for this tile's ECEF origin
        _, _, tile_alt = lv95_lhn02_to_wgs84_3d.transform(g["cx"], g["cy"], tile_alt_lhn02)

        # Recalculate ECEF parameters using the correct ellipsoidal terrain altitude for this tile
